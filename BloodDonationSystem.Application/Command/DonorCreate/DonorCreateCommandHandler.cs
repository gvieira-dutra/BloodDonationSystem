...
namespace BloodDonationSystem.Application.Command.DonorCreate
{
    public class DonorCreateCommandHandler : IRequestHandler<DonorCreateCommand, int>
    {
        ...
        {            
            var address = new Address(request.Address.Street, request.Address.City, request.Address.Province, request.Address.PostalCode);
            await _dbContext.Addresses.AddAsync(address);
            await _dbContext.SaveChangesAsync();

            var donor = new Donor(request.FullName, request.Email, request.DoB, request.Gender, request.Weight, request.BloodType, request.RhFactor, address.Id);
